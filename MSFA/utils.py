...
def create_generator(opt):
    ...
    generator = HST_KAN_FPN2.HST(opt)                 # STP-KAN
    ...
    # else:
    #     # 通过修改此句话来确定训练的是先RDB再RGB的网络，还是先RGB再RDB的网络。
    #     RF_Unet.weights_init(generator, init_type = opt.init_type, init_gain = opt.init_gain)
...
def load_dict(process_net, pretrained_net):
    ...
    process_net.load_state_dict(process_dict, strict=False)
    return process_net

##+======================================================================我加的
def initialize_logger(file_dir):
    logger = logging.getLogger()     # Logger 记录器 loss
    fhandler = logging.FileHandler(filename=file_dir, mode='a')       # logging.FileHandler；    日志输出到文件。
    formatter = logging.Formatter('%(asctime)s - %(message)s',"%Y-%m-%d %H:%M:%S")
    # formatter = logging.Formatter(fmt=None, datefmt=None)
    # 其中，fmt是消息的格式化字符串，datefmt是日期字符串。如果不指明fmt，将使用’%(message)s’。
    # 如果不指明datefmt，将使用ISO8601日期格式。
    fhandler.setFormatter(formatter)    #设置一个格式化器formatter
    logger.addHandler(fhandler)     # 为Logger实例增加一个处理器
    logger.setLevel(logging.INFO)   #设置日志级别为INFO
    return logger

def record_loss(loss_csv, epoch, iteration, train_loss):
    """ Record many results."""
    loss_csv.write('{},{},{}\n'.format(epoch, iteration, train_loss))         #  epoch() * iteration(),
    loss_csv.flush()      # 作用是每写入一句就立即保存一次，哪怕程序突然断掉也不会影响。
    # 在写入的程序运行的同时可以随时打开文本查看写入的数据，并且省内存。
    loss_csv.close     # loss_csv.close（）不对  。  ？？？
...
def reconstruction_patch_image_gpu(rgb, model, patch, stride):
    all_time = 0
    _, _, w, h = rgb.shape
    rgb = torch.from_numpy(rgb).float()      # torch.from_numpy
...
# ----------------------------------------
#             PATH processing   路径处理         用于compute_MRAE.py
# ----------------------------------------
def check_path(path):
...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    # Output-space coordinates   输出空间坐标
